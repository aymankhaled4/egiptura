...
import { CrossedSceptersIcon, RaSunIcon, CrescentMoonIcon, CalendarIcon, MapPinIcon, ShipIcon, DiamondIcon, WhatsAppIcon } from './Icons';
...
const ProgramModal: React.FC<ProgramModalProps> = ({ program, onClose, detectedSeason }) => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'gold' | 'diamond' | null>(
  ...
  const [selectedItinerary, setSelectedItinerary] = useState<ItineraryItem[]>(() => {
    if (program.itinerary && Array.isArray(program.itinerary) && program.itinerary.length > 0) {
      return program.itinerary;
    }
    if (program.itineraryOptions?.[0]?.itinerary && Array.isArray(program.itineraryOptions[0].itinerary)) {
            ...
            return {
              ...
              content: (
                <ul className="list-disc list-inside space-y-2 text-gray-400 text-sm">
                  {activitiesList.map((act, i) => <li key={i}>{act}</li>)}
                </ul>
              ),
            };
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-3">
              {program.accommodations?.gold && program.accommodations.gold.length > 0 ? (
                program.accommodations.gold.map((acc, index) => (
                  <div key={`gold-${index}`} className="bg-gray-800/50 p-4 rounded-lg border border-gray-700 flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-200">{acc.city?.[lang] ?? acc.city?.en ?? 'Unknown'}</p>
                      <p className="text-sm text-gray-400">{acc.hotel?.[lang] ?? acc.hotel?.en ?? 'Hotel'}</p>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-gray-400 text-sm text-center py-4">No accommodation information available</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {program.accommodations?.diamond && program.accommodations.diamond.length > 0 ? (
